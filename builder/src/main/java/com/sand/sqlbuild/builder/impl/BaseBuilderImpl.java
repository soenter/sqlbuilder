...
public class BaseBuilderImpl<T extends BaseBuilder> implements BaseBuilder<T> {
	...
	public T distinct(Fieldable... fields) {
		return (T)distinct().fields(fields);
	}

	/*------------------------------------插入（insert）------------------------------------*/

	public T insert(Class<? extends Table> clazz, Field<?>... fields) {
		Table table = getCachedTable(clazz);
	...
	public T insert (Class<? extends Table> clazz, Setter<?>... setters) {
		return insert(getCachedTable(clazz).getName(), setters);
	}
	...
	public T upsertOracle (Class<? extends Table> clazz, Setter<?>[] setters, Filter<?>[] filters) {
		...
		String tableName = getCachedTable(clazz).getName();
    ...
    public T from(Class<? extends Table> first, Class<? extends Table>... clazzs) {
        Table table = getCachedTable(first);
        builder.append(" from ").append(table.getName());
        for (Class<? extends Table> class1 : clazzs) {
            //FIXME 获取表名，改为注解形式
            comma();
            table = getCachedTable(class1);
	...
	public T from(Class<? extends Table>[] clazzs) {
		...
		for (Class<? extends Table> class1 : clazzs) {
			//FIXME 获取表名，改为注解形式
			Table table = getCachedTable(class1);
			if(isFirst){
				isFirst = false;
			} else {
				comma();
	...
	private static final ConcurrentHashMap<Class, TableWarp> TABLE_CACHE = new ConcurrentHashMap<Class, TableWarp>();

	private static Table getCachedTable (Class<? extends Table> clazz){

		TableWarp cachedTableWarp = TABLE_CACHE.get(clazz);

		if(cachedTableWarp != null){
			return cachedTableWarp.getTable();
		}

		TableWarp newTableWarp = new TableWarp(clazz);
		TableWarp prevTableWarp = TABLE_CACHE.putIfAbsent(clazz, newTableWarp);

		if(prevTableWarp == null){
			return newTableWarp.createTable();
		}

		return prevTableWarp.getTable();
	}
	...
	private static class TableWarp{
		...
		public TableWarp (Class<? extends Table> clazz) {
			this.clazz = clazz;
		}
		...
		public Table createTable(){
			return table = BuilderUtils.newTable(clazz);
		}
	}
